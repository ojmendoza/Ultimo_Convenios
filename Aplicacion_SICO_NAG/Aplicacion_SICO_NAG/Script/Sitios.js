...
$(document).ready(function () {
    ...
    function consultar() {
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/ObtenerSitios",
            data: JSON.stringify({ "Id_Sitios": 1 }),
    ...
    //FUNCION DE GUARDAR 
    function GuardarSitios(callback) {
        var datos = {};
        ...
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/GuardarSitios",
            data: JSON.stringify({ 'datosSitios': datos }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                if (response.d == "True") {
    ...
    //Funcion para Actualizar todos los datos 
    function Update() {
        var datos = {};
        datos.Id = $("[id*=id]").val();        
        datos.Fecha_Instalacion = $('.datepicker').val();        
        ...
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/UpdateDatosSitios",
            data: JSON.stringify({ 'datosSitios': datos }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                if (response.d == "True") {
                    ...
                    Materialize.toast('No se actualizaron los datos. ' + response.d + '', 6000, 'red');
                }
            },
            failure: function (response) {
                Materialize.toast('ERROR, intente nuevamente. ', 4000, 'rounded');
    ...
    $('.btn_GuardarS1').click(function (e) {
        e.preventDefault();      
    ...
    function Modificar_datos() {
        ...
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/ModificarDatos",
            data: JSON.stringify({ "Id_sitios": datos }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
    ...
    // Boton actualizar llama los datos a los textbox a modificar
    $(document).on("click", '.btn_Actualizar', function (e) {
        e.preventDefault();
        var row = $(this).parent().parent()[0];
        var data = tabla.row($(this).parents("tr")).data();
        var Id_Sitios = $("[id*=id]").val(data.Id);
        $("[id*=txt_Sitio]").val(data.Centro);
        $("[id*=tipoSitio]").val(data.Sace);  
        Modificar_datos();
    });
    ...
    function ActivarEstado() {
        ...
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/ActivarEstado",
            data: JSON.stringify({ 'IdSitios': datos }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                if (response.d == "True") {
    ...
    function DesactivarEstado() {
        ...
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/DesactivarEstado",
            data: JSON.stringify({ 'IdSitios': datos }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
    ...
    // Boton para cancelar 
    $('#btn_CancelarS').click(function (e) {
        limpiar();
        actualizar();
    });
    ...
    function ObtenerDatosDrop() {
        ...
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/ObtenerDatosDrop",
            data: JSON.stringify({ 'dropInciso': dropCatalogo }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (r) {
                var depto = $("[id*=DDL_deptoS]");
                $.each(r.d, function () {
    ...
    $('.DDL_departamentoS').change(
        function (e) {
            e.preventDefault();
            ObtenerDatosDropM();
            $('select').material_select('destroy');
            $('.DDL_MunicipioS option').remove();
            $('select').material_select();
    ...
    function ObtenerDatosDropM() {
        var dropCatalogo = $("[id*=DDL_deptoS]").val();
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/ObtenerDatosDropM",
            data: JSON.stringify({ 'dropInciso': dropCatalogo }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (r) {
                var depto = $("[id*=Ddl_MuniS]");
                $.each(r.d, function () {
                    depto.append($("<option></option>").val(this['Value']).html(this['Text']));
                });
    ...
    function ObtenerDatosOperador() {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ 'dropInciso': $("[id*=id_inst]").val() }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (r) {
    ...
    function ObtenerDatos_Solicitudes() {
        $.ajax({
            type: "POST",
            url: "/vistas/Sitios.aspx/modal_solicitudes",
            data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (r) {

                tabla = $("#datosolic").DataTable({
                    "scrollX": true,
                    "language": {
                        "lengthMenu": "",
                        "zeroRecords": "No se encontraron resultados en su busqueda",
    ...
    $(document).on('click', '.btn_Datos_solic', function (event) {
        ...
        $("[id*=txt_Sitio]").val(data.Nombre_Institución);
        $("[id*=tipoSitio]").val(data.Tipo_Institución);         
        $('select').material_select();
    });
